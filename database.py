...
def initialize_connection_pool():
    ...
    try:
        if 'dsn' in connection_params:
            connection_pool = pool.ThreadedConnectionPool(min_conn, max_conn, dsn=connection_params['dsn'])
        else:
...
# Initialize the connection pool at module import time
...
class Database:
    ...
    def get_unsynced_blocks_for_primary(self, primary_account_id):
        ...
        try:
            ...
            cursor.execute(f"""
                SELECT ba.did, ba.handle, ba.reason, ba.id as original_block_id
                FROM blocked_accounts{self.table_suffix} ba
                JOIN accounts{self.table_suffix} a ON ba.source_account_id = a.id
